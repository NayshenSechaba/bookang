...
const BusinessProfile = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-muted via-background to-blue-muted flex items-center justify-center">
        <div className="text-blue text-xl">Loading...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-muted via-background to-blue-muted">
      {/* Cover Image */}
      <div className="relative h-64 w-full">
        {business.banner_url ? (
          ...
          <div className="w-full h-full bg-gradient-to-r from-blue-muted to-blue" />
        )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative">
        <div className="flex flex-col sm:flex-row items-start sm:items-end gap-6 mb-8">
          ...
          <div className="flex-1 bg-card/95 backdrop-blur rounded-lg p-6 shadow-lg">
            <h1 className="text-3xl font-bold text-card-foreground mb-2">
              {business.business_name || business.full_name}
            </h1>
            <p className="text-muted-foreground mb-4">{business.business_description}</p>
            ...
            <div className="flex gap-3 mt-4">
              {business.instagram_url && (
                <a
                  ...
                  className="p-2 rounded-full bg-blue-muted text-blue hover:bg-blue/10 transition-colors"
                >
            ...
            {/* Book Now Button */}
            <Button className="mt-6 bg-blue hover:opacity-90 text-blue-foreground">
        ...
        <Tabs defaultValue="services" className="mb-12">
          <TabsList className="grid w-full grid-cols-4 mb-8">
            <TabsTrigger value="services" className="data-[state=active]:bg-blue data-[state=active]:text-blue-foreground">
              Services
            </TabsTrigger>
            <TabsTrigger value="portfolio" className="data-[state=active]:bg-blue data-[state=active]:text-blue-foreground">
              Portfolio
            </TabsTrigger>
            <TabsTrigger value="reviews" className="data-[state=active]:bg-blue data-[state=active]:text-blue-foreground">
              Reviews
            </TabsTrigger>
            <TabsTrigger value="hours" className="data-[state=active]:bg-blue data-[state=active]:text-blue-foreground">
              Hours & Location
            </TabsTrigger>
          </TabsList>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {services.length === 0 ? (
                ...
                services.map((service) => (
                  <Card key={service.id} className="border-blue/20 hover:shadow-lg transition-shadow">
                    <CardHeader>
                      <CardTitle className="text-blue">{service.name}</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-muted-foreground text-sm mb-4">{service.description}</p>
                      <div className="flex justify-between items-center">
                        <Badge className="bg-blue text-blue-foreground">R{service.price}</Badge>
          ...
          <TabsContent value="reviews">
            <div className="space-y-4">
              {reviews.length === 0 ? (
                ...
                reviews.map((review) => (
                  <Card key={review.id} className="border-blue/20">
                    <CardContent className="pt-6">
                      <div className="flex items-start gap-4">
                        <Avatar>
                          <AvatarImage src={review.profiles?.avatar_url} />
                          <AvatarFallback className="bg-blue text-blue-foreground">
                  ...
                  <div className="space-y-2">
                    {business.business_hours &&
                      Object.entries(business.business_hours).map(([day, hours]: [string, any]) => (
                        <div key={day} className="flex justify-between py-2 border-b border-blue/10 last:border-0">
                          <span className="font-medium text-foreground">{getDayName(day)}</span>
                          <span className="text-muted-foreground">
                            {hours.closed ? (
                              <Badge variant="outline" className="border-blue text-blue">Closed</Badge>
                            ) : (
              ...
              <Card className="border-blue/20">
                ...
                <CardContent>
                  {business.latitude && business.longitude ? (
                    <div className="h-64 bg-blue-muted rounded-lg flex items-center justify-center">
                      <p className="text-blue">Map view (requires Mapbox integration)</p>
                    </div>
                  ) : (
